...
export const Insights = () => {
  ...
  const analysis = useMemo(() => {
    ...
    const suggestionMap = {
      Rent: "Housing is your fixed cost and it is currently your highest expense.",
      Food: "Dining out is peaking. Switching to a weekly meal plan could save you some amount.",
      Travel: "Commute costs are high. Look into a monthly pass for better rates.",
      Education: "Great investment! Keep receipts for potential tax credits.",
      Misc: "Uncategorized spending is high. Try to label these to see where the leak is.",
      None: "No expense data found."
    };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
      <div className="dashboard-card p-6 bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-teal-50 dark:bg-teal-500/10 rounded-lg text-teal-600 border border-teal-100 dark:border-teal-500/20">
            <Lightbulb size={20} />
          </div>
          <h3 className="font-bold text-slate-900 dark:text-white">Top Spending Insight</h3>
        </div>
        <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm">
          Highest expenditure is in <span className="text-teal-600 font-bold px-1 bg-teal-50 dark:bg-teal-500/10 rounded">{analysis.topCat}</span>, totaling <span className="text-slate-900 dark:text-white font-bold">₹{analysis.maxVal.toLocaleString()}</span>. 
        </p>
        <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 text-[10px] text-teal-600 font-black tracking-widest uppercase">
          Suggestion: {analysis.suggestion}
        </div>
      </div>
      ...
      <div className="dashboard-card p-6 bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-rose-50 dark:bg-rose-500/10 rounded-lg text-rose-600 border border-rose-100 dark:border-rose-500/20">
            <TrendingDown size={20} />
          </div>
          <h3 className="font-bold text-slate-900 dark:text-white">Efficiency Score</h3>
        </div>
        ...
        <div className="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full mt-4 overflow-hidden">
            <div className="bg-rose-500 h-full" style={{ width: `${Math.min(analysis.burnRate, 100)}%` }}></div>
        </div>
      </div>
    </div>
  );
