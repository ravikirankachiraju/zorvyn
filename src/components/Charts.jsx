...
// import { TREND_DATA, TRANSACTIONS } from '../data/mockData';
...
export const DashboardCharts = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
      <div className="dashboard-card p-6 h-[350px] bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
        <h3 className="font-bold mb-4 dark:text-white">Balance Flow</h3>
        <ResponsiveContainer width="100%" height="90%">
          <AreaChart data={TREND_DATA}>
            ...
            <Area type="monotone" dataKey="bal" stroke="#0d9488" fill={darkMode ? "#0d948820" : "#ccfbf1"} strokeWidth={3} />
          </AreaChart>
        </ResponsiveContainer>
      </div>

      <div className="dashboard-card p-6 h-[350px] bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
        <h3 className="font-bold mb-4 dark:text-white">Spending Split</h3>
        <ResponsiveContainer width="100%" height="90%">
          <PieChart>
            <Pie data={pieData} innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
              {pieData.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
