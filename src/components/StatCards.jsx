...
export const StatCards = () => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {data.map((item, i) => (
        <div key={i} className="dashboard-card p-6 flex justify-between items-center bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
          <div>
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">{item.label}</p>
            <p className="text-3xl font-bold mt-1 dark:text-white">₹{item.val.toLocaleString()}</p>
          </div>
          <div className={`p-4 rounded-2xl ${item.bg} dark:bg-opacity-10 ${item.color}`}>{item.icon}</div>
        </div>
      ))}
    </div>
  );
