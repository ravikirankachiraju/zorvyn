...
export const Footer = () => {
  ...
  const dynamicInsight = useMemo(() => {
    if (!transactions || transactions.length === 0 || stats.expense === 0) return { category: 'None', percentage: 0 };
    const totals = {};
    transactions.filter(t => t.type === 'expense').forEach(t => totals[t.category] = (totals[t.category] || 0) + t.amount);
    let top = Object.entries(totals).reduce((a, b) => b[1] > a[1] ? b : a, ["None", 0]);
    return { category: top[0], percentage: ((top[1] / stats.expense) * 100).toFixed(0) };
  }, [transactions, stats.expense]);

  return (
    <footer className="dashboard-card p-6 mt-12 mb-12 border-l-4 border-slate-300 bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 transition-colors">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-2 h-2 rounded-full bg-teal-500 animate-pulse"></div>
        <h4 className="font-bold text-[10px] uppercase tracking-[0.2em] text-slate-400">System Insight</h4>
      </div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="text-slate-600 dark:text-slate-400 text-sm font-medium leading-relaxed max-w-3xl">
          {role === 'Admin' ? (
            <p>Critical Spending Alert: Your <span className="text-slate-900 dark:text-white font-bold">'{dynamicInsight.category}'</span> category accounts for <span className="text-slate-900 dark:text-white font-bold">{dynamicInsight.percentage}%</span> of outgoings.</p>
          ) : (
            <p>Read-Only Mode: Contact administrator for edit permissions.</p>
          )}
        </div>
        {role === 'Admin' && (
           <button onClick={() => exportToExcel(transactions)} className="px-4 py-2 bg-white dark:bg-slate-800 text-teal-700 dark:text-teal-400 text-xs font-bold rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-teal-600 hover:text-white transition-all">Export Report</button>
        )}
      </div>
    </footer>
  );
